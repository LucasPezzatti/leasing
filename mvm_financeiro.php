...
<div class="card mt-3"  style="max-width:840px" >
    <div class="card-content">
        ...
        <?php 
            ...
            ?>
          <div class="col-8 col-lg-6 col-xl-6 border-light">
                <div class="card-body">
                  <h5 class="text-danger shadow_danger mb-0">R$ <?= $totaldesp; ?> <span class="float-center"><label for="dateinput"  class="btn btn-sm btn-light"> <?php echo $showperiodo;?>  <i class="zmdi zmdi-calendar"></i></label></span></h5>
            ...
            <div class="col-8 col-lg-6 col-xl-6 border-light">
                <div class="card-body">
                  <h5 class="text-success shadow_dark mb-0">R$ <?= $totalrece; ?> <span class="float-center text-success"><label for="dateinput"  class="btn btn-sm btn-light"> <?php echo $showperiodo;?>  <i class="zmdi zmdi-calendar"></i></label></span></h5>
 ...
 <?php 
 $localsql=new localsql();
 $buscadesp = $localsql->busca_mvm_financeiro_all($dtini,$dtfin,$pdom); //BUSCA DESPESAS (Exceto BAIXAS)
 foreach($buscadesp as $line){
  $seq=$line['seq'];	
...
?>
	...
	<div class="row">
	 <div class="col-12 col-lg-12">
	   <div class="card">
	     <div class="card-header">Movimento Financeiro
		 </div>
     <div class="card-action">
       <span class="float-right">
        <br>
        <form class="form form-inline" method="POST" action="global_date_pr.php?origin=mvmfinanc">
       <a href="mvm_receita.php?action=new&key=<?php echo md5(gethostbyaddr($_SERVER['REMOTE_ADDR']));?>"> <button class="btn btn-success" type="button" style="margin-right:8px" >Novo Lançamento De Entrada </button> </a>
       <a href="mvm_despesa.php?action=new&key=<?php echo md5(gethostbyaddr($_SERVER['REMOTE_ADDR']));?>"> <button class="btn btn-danger" type="button" style="margin-right:8px" >Novo Lançamento De Saída </button> </a>
...
?>     
                    ...
                    <td style="font-size:13px;"><?php if($tipomvm=="E"){?><button class="btn btn-success"><?php echo number_format($valor, 2, ',', '.');?> R$ </button><?php } ?></td>
                    <td style="font-size:13px;"><?php if($tipomvm=="S"){?><button class="btn btn-danger"><?php echo number_format($valor, 2, ',', '.');?> R$ <?php } ?></button></td>
                 ...
                 <td class="cursor_pointer"><a class="btn btn-info" href="mvm_despesa.php?id=<?php echo $seq;?>&action=edt&key=<?php echo md5(gethostbyaddr($_SERVER['REMOTE_ADDR']));?>">
