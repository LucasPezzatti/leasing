...
<div class="row">
	 <div  class="col-12 col-lg-12">
	   <div id="relatorio" class="card printcolor">
	     <div class="card-header printcolor" style="text-align:center;font-size:22px;border-bottom:0px;!important"> Relatório De Títulos a Vencer <br>
...
$busca = $localsql->relatorio2_titulos_a_vencer($pdom); 
...
?>                    
            ...
            <?php 
            $busca = $localsql->relatorio2_sum_che_avencer($pdom); 
            foreach($busca as $line){
            $tot_venc_che=$line['tot_venc_che'];	
            }
            $busca = $localsql->relatorio2_sum_dup_avencer($pdom); 
            foreach($busca as $line){
              $tot_venc_dup=$line['tot_venc_dup'];	
            } 
            $busca = $localsql->relatorio2_sum_emp_avencer($pdom); 
            foreach($busca as $line){
            ...
            ?>
             ...
             <table class="table table-bordered align-items-center printcolor">
                 <tr><td>CHEQUES A VENCER </td> <td><?= number_format($tot_venc_che, 2, ',', '.');?> R$</td></tr>
                 <tr><td>DUPLICATAS A VENCER </td> <td><?= number_format($tot_venc_dup, 2, ',', '.');?> R$</td></tr>
                 <tr><td>CARNÊS A VENCER </td> <td><?= number_format($tot_venc_emp, 2, ',', '.');?> R$</td></tr>
                 <tr><td style="border:none!important"></td><td style="border:none!important"></td></tr>
                 <tr><td>TOTAL A VENCER </td> <td><?= number_format($total_avencer, 2, ',', '.');?> R$</td></tr>
             </table>    
             </div>
            </div>
